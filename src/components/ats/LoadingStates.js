        ...
        {stages.map((s, index) => (
          <div 
            ...
            className={`flex flex-col items-center ${
              index <= currentStageIndex ? 'text-[var(--accent)]' : 'text-gray-400'
            }`}
          >
...
export const FloatingActionButton = memo(({ onClick, disabled, children, className = "" }) => (
  <button
    onClick={onClick}
    disabled={disabled}
...
export const Toast = memo(({ 
  ...
  return (
    <div className={`fixed top-4 right-4 z-50 max-w-sm w-full border rounded-lg p-4 shadow-lg transform transition-all duration-300 ${colors[type]}`}>
