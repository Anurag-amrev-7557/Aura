...
import { Moon, Sun, ArrowUpRight, User as UserIcon, FileText, LayoutDashboard, Settings as SettingsIcon, LogOut, Menu as MenuIcon, X as CloseIcon, Briefcase, Bot, CreditCard, Info, Mail } from "lucide-react";
import { motion, useScroll, useSpring, useTransform, AnimatePresence, MotionConfig } from "framer-motion";
...
// Highly optimized NavLink component with React.memo
...
// Highly optimized UserAvatar component with React.memo
const UserAvatar = memo(({ user, onClick, menuOpen, effectiveTheme }) => {
  ...
  // Memoize the gradient background to prevent recalculation
  ...
  return (
    <div className="relative inline-flex items-center justify-center">
      {/* Animated ring around user avatar - perfectly centered */}
      <motion.div
      ...
      <button
        type="button"
        onClick={onClick}
        className="relative inline-flex cursor-pointer items-center justify-center h-9 w-9 rounded-full border border-[var(--border)] overflow-hidden bg-[var(--muted)] hover:opacity-90 transition focus:outline-none z-10"
        aria-haspopup="menu"
...
// Highly optimized ThemeToggle component with React.memo
...
// Main navigation items configuration - memoized to prevent recreation
...
function Navbar() {
  ...
  // Optimized scroll animations with reduced calculations
  const { scrollY } = useScroll();
  const progressRaw = useTransform(scrollY, [0, 100], [0, 1]);
  const progress = useSpring(progressRaw, { 
    stiffness: 150, 
    damping: 24, 
    mass: 0.28,
    restDelta: 0.001 // Prevent unnecessary updates
  });

  // Animation values - properly using useTransform at top level
  const radius = useTransform(progress, [0, 1], [20, 999]);
  const pad = useTransform(progress, [0, 1], [24, 12]);
  
  // Memoized theme-dependent color arrays
      ...
      >
        ...
        <div className="flex items-center gap-3">
          ...
          >
              ...
              <svg width="32" height="32" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
                <defs>
                  <linearGradient id="logoGradient" x1="0%" y1="0%" x2="100%" y2="100%">
                    <stop offset="0%" style={{stopColor: effectiveTheme === "dark" ? "#60A5FA" : "#3B82F6", stopOpacity: 1}} />
                    <stop offset="100%" style={{stopColor: effectiveTheme === "dark" ? "#3B82F6" : "#1E40AF", stopOpacity: 1}} />
                  </linearGradient>
            ...
            <span className="font-semibold tracking-tight text-[19.2px]">Aura</span>
          </Link>
        </div>

        {/* Desktop Navigation */}
        <nav className="hidden md:flex items-center gap-8 text-[15px]" role="navigation">
          {desktopNavLinks}
        </nav>
        ...
        <div className="hidden md:flex items-center gap-3">
          ...
          {status === "loading" ? (
            <div className="h-9 w-9 rounded-full bg-[var(--foreground)]/10 animate-pulse" aria-hidden />
          ) : session?.user ? (
