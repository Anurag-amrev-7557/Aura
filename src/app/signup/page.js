...
import { Mail, Lock, User, Loader2, ArrowRight, Check, Github, Eye, EyeOff } from "lucide-react";
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 grid lg:grid-cols-2 gap-10 items-center">
        ...
        >
          <div className="inline-flex items-center gap-2 rounded-full border border-[var(--border)] px-3 py-1 text-xs text-[var(--foreground)]/70">
            <span className="h-1.5 w-1.5 rounded-full bg-blue-500" /> Start free — no credit card
          </div>
          <h1 className="mt-4 text-3xl sm:text-4xl font-semibold tracking-tight">Create your account</h1>
          <p className="mt-3 text-[var(--foreground)]/70 max-w-md">
            Personalize your job search and accelerate your applications with AI.
          </p>
            ...
            <div className="relative p-6">
              ...
              <div className="space-y-4">
                <div className="relative">
                  ...
                  />
                  <User size={16} className="absolute right-3 top-1/2 -translate-y-1/2 opacity-60" />
                </div>
                <div className="relative">
                ...
                <div className="text-[10px] text-[var(--foreground)]/70" id="password-hint">
                  Use at least 8 characters, with a number, uppercase, and symbol.
                </div>
                ...
                {confirmPassword && confirmPassword !== password && (
                  <div className="text-xs text-red-600 dark:text-red-400">Passwords do not match.</div>
                )}
                <div className="flex items-center gap-2 text-xs text-[var(--foreground)]/80">
                  <input id="terms" type="checkbox" className="h-4 w-4 rounded border-[var(--border)]" checked={agreed} onChange={(e)=>setAgreed(e.target.checked)} />
                  <label htmlFor="terms">I agree to the <Link href="#" className="underline underline-offset-4">Terms</Link> and <Link href="#" className="underline underline-offset-4">Privacy Policy</Link></label>
                </div>
              </div>
              <div className="mt-2 min-h-[20px] text-xs text-red-600 dark:text-red-400" aria-live="polite" aria-atomic="true">{error}</div>
                <button
                  type="submit"
                  disabled={isSubmitting || !name.trim() || !isValidEmail(email) || password.length<6 || confirmPassword!==password || !agreed}
              ...
              Already have an account? <Link href="/login" className="underline underline-offset-4 hover:opacity-80">Sign in</Link>
            </div>
            </div>
          </div>
        </motion.div>
        ...
        >
          <div className="relative mt-44 overflow-hidden rounded-3xl border border-[var(--border)] bg-[var(--background)] p-6 min-h-[460px] h-full">
            ...
            <div className="relative">
              <h3 className="text-xl font-semibold tracking-tight">Why join Aura?</h3>
            <ul className="mt-4 space-y-3 text-sm text-[var(--foreground)]/80">
              <li className="flex items-center gap-2"><Check size={16} /> Smart job matches</li>
              <li className="flex items-center gap-2"><Check size={16} /> ATS-ready resume feedback</li>
              <li className="flex items-center gap-2"><Check size={16} /> Save and track applications</li>
              <li className="flex items-center gap-2"><Check size={16} /> Email alerts and reminders</li>
            </ul>
            </div>
          </div>
        </motion.div>
